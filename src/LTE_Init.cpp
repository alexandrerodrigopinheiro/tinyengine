...
#include "../include/TinyEngine/LTE_Globals.h"
#include "../include/TinyEngine/LTE_Init.h"
...
bool LTE_Initialize(const LTE_Config& config) {
...
#else
	uint32_t videoFlags = config.video.flags.enable_hardware_surface
		|| (videoInfo != NULL && videoInfo->hw_available)
	...
	int width = videoInfo != NULL ? videoInfo->current_w : config.window.width;
	int height = videoInfo != NULL ? videoInfo->current_h : config.window.height;
	...
	std::cout << "Video mode: " << LTE_GLOBAL_SCREEN->w << "x" << LTE_GLOBAL_SCREEN->h
			<< " - " << (int)LTE_GLOBAL_SCREEN->format->BitsPerPixel << " bpp" << std::endl;
...
void LTE_Shutdown() {
#ifdef USE_OPENGL
	FTGLFontManager::instance().LTE_CleanUp();
#endif
